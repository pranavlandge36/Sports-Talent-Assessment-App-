// lib/activities/vertical_jump_page.dart
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:firebase_auth/firebase_auth.dart';
import 'package:flutter/material.dart';

import 'vertical_jump_record_page.dart';
...
class _VerticalJumpPageState extends State<VerticalJumpPage> {
  ...
  Future<void> _loadPersonalBestAndLeaderboard() async {
    ...
    try {
      ...
      if (bestDoc.exists) {
        ...
        // fallback to highest assessment
        final q = await FirebaseFirestore.instance
            .collection('assessments')
            .where('userId', isEqualTo: user.uid)
            .where('activityKey', isEqualTo: 'vertical_jump')
      ...
      // Leaderboard from best_scores if available
      ...
      if (bestScoresSnapshot.docs.isNotEmpty) {
        _leaderboard = bestScoresSnapshot.docs.map((doc) {
          ...
          return _LeaderboardItem(
            ...
            name: (d['displayName'] ?? '')
                .toString(), // may be email; we'll enrich below
            score: (d['score'] is num) ? (d['score'] as num).toInt() : 0,
            timestamp: d['timestamp'],
          );
        }).toList();

        // Enrich names from users collection (non-fatal)
      ...
      debugPrint('Failed to load leaderboard (vertical jump): $e\n$st');
      await _computeLeaderboardFallback();
    } finally {
  ...
  // Replace leaderboard item names with users/{uid}.name or users/{uid}.displayName when available.
  // Fallback priority used later in UI: canonicalName -> original displayName -> email -> userId.
  Future<void> _enrichNamesForLeaderboard() async {
    try {
      ...
      debugPrint('Failed to enrich vertical jump leaderboard names: $e');
    }
  }
  ...
  Future<void> _computeLeaderboardFallback() async {
    ...
    // attempt to enrich with users collection
    await _enrichNamesForLeaderboard();
  ...
  Future<void> _onRecordPressed() async {
    final result = await Navigator.push<bool>(
      context,
      MaterialPageRoute(builder: (_) => const VerticalJumpRecordPage()),
    );
    if (result == true) {
  ...
  // Return the best readable label for a row
  String _labelForRow(_LeaderboardItem row) {
    final n = (row.name ?? '').toString().trim();
    if (n.isNotEmpty) return n;
    // try email field if present (some assessments include it)
    final email = (row.email ?? '').toString();
    if (email.isNotEmpty) return email;
    return row.userId;
  }

  // Leading widget: medal for top 3 with nicer colors
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    _personalBest != null
                        ? '$_personalBest cm • $_personalBestWhen'
                  ...
                  const SizedBox(height: 8),
                  const Text(
                    'Place phone on a stable surface so full body and feet are visible.',
                    style: TextStyle(color: Colors.white70),
                  ),
                ],
              ),
...
class _LeaderboardItem {
  final String userId;
  final String? name; // may be null until enriched
  final int score;
