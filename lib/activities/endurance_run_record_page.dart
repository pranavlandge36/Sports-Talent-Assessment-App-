// lib/activities/endurance_run_record_page.dart
import 'dart:async';
import 'dart:io';
import 'dart:convert';
...
class EnduranceRunRecordPage extends StatefulWidget {
  /// Records an endurance run attempt (e.g. 1.6 km).
  /// UX mirrors PushUpRecordPage: user starts recording, timer runs, stops to save.
  const EnduranceRunRecordPage({super.key});

  @override
  State<EnduranceRunRecordPage> createState() => _EnduranceRunRecordPageState();
}

class _EnduranceRunRecordPageState extends State<EnduranceRunRecordPage>
  ...
  // Stopwatch for elapsed run time
  ...
  Future<void> _stopRecordingAndAnalyze() async {
    ...
    try {
      // Final score = elapsed seconds (integer). Lower is better for run.
      final elapsedSec = _stopwatch.elapsed.inSeconds;

      await _saveAssessmentMetadata(
        activityKey: 'endurance_run',
        score: elapsedSec,
        unit: 's',
      ...
      setState(() => _isProcessing = false);
      // remove local file to avoid storage bloat
      try {
  ...
  Future<void> _saveAssessmentMetadata({
    ...
    // Update best_scores doc atomically. For endurance run LOWER time is better.
    final bestRef = FirebaseFirestore.instance
        .collection('best_scores')
        .doc('${user.uid}_endurance_run');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Record Endurance Run (1.6 km)'),
        backgroundColor: Colors.black,
        elevation: 1,
      ),
      body: SafeArea(
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  const Text(
                    'Place phone so runner and track are visible for entire attempt.',
                    style: TextStyle(color: Colors.white70),
                  ),
                ],
              ),
