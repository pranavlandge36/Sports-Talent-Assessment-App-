...
import 'package:sports/ml/pushup_counter.dart';

class PushUpRecordPage extends StatefulWidget {
  const PushUpRecordPage({super.key});

  @override
  State<PushUpRecordPage> createState() => _PushUpRecordPageState();
}

class _PushUpRecordPageState extends State<PushUpRecordPage>
  ...
  Future<void> _onFrame(CameraImage image) async {
    ...
    try {
      ...
      // REQUIRED joints for push-ups
      if ((!keypoints.containsKey('left_shoulder') &&
              !keypoints.containsKey('right_shoulder')) ||
          (!keypoints.containsKey('left_elbow') &&
              !keypoints.containsKey('right_elbow')) ||
          (!keypoints.containsKey('left_wrist') &&
  ...
  Future<void> _saveAssessment() async {
    ...
    final fingerprint = sha256
        .convert(utf8.encode('pushups|$_reps|$attemptId|${user.uid}'))
        .toString()
        .substring(0, 32);
    ...
    await FirebaseFirestore.instance.collection('assessments').add({
      ...
      'score': _reps,
      'unit': 'reps',
      'modelVersion': 'movenet-pushup-v1',
      'confidence': 0.9,
      'cheatScore': 0.05,
      'attemptId': attemptId,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: Colors.black,
      appBar: AppBar(title: const Text('Push-ups')),
      body: Column(
        children: [
